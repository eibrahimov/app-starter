...
- **`cx` helper** (`components/ui/cx.ts`) merges class names — no `clsx`/`cva`.
...
- **`Button`** `{ variant?: "primary" | "ghost" | "danger" } & button attrs` —
  defaults to `type="button"`. `ghost`/`danger` are the inline text-button rows.
